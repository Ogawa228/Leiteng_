...
class MyHandler(FileSystemEventHandler):
    def __init__(self, target_path, naming_rule):
        ...
        if not event.is_directory:
            ...
            for retry in range(max_retries):
                try:
                    ...
                    logging.info(f'重命名文件 {file_path} 为 {new_file_path} 成功')
                    break
                except OSError as e:
                    logging.error(f'重命名文件 {file_path} 时出现错误: {e}')
                    if retry < max_retries - 1:
                        time.sleep(retry_delay)
                    else:
                        logging.error(f'重命名文件 {file_path} 失败，经过 {max_retries} 次重试')
...
class TrayApp(QObject):
    ...
    def __init__(self):
        ...
        # 创建快捷键
        self.start_shortcut = QShortcut(QKeySequence('F7'), self.app.activeWindow())
        self.start_shortcut.activated.connect(self.start_auto_naming)
        self.stop_shortcut = QShortcut(QKeySequence('F8'), self.app.activeWindow())
    ...
    def load_startup_status(self):
        try:
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0,
                                 winreg.KEY_READ)
            try:
                value, value_type = winreg.QueryValueEx(key, self.app.applicationName())
    ...
    def set_startup(self, enable):
        if enable:
            ...
            winreg.SetValueEx(key, self.app.applicationName(), 0, winreg.REG_SZ, script_path)
            winreg.CloseKey(key)
        else:
            try:
                key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0,
                                     winreg.KEY_ALL_ACCESS)
                winreg.DeleteValue(key, self.app.applicationName())
                winreg.CloseKey(key)
            except FileNotFoundError:
                pass
    ...
    def start_auto_naming(self):
        if self.is_running:
            print("自动命名已经在运行中，无需重复启动")
            return
        if not self.is_running:
            event_handler = MyHandler(self.target_path, self.naming_rule)
            try:
                ...
                print(f"启动监控时出现错误: {e}")
                logging.error(f"启动监控时出现错误: {e}")

    def stop_auto_naming(self):
        if not self.is_running:
            print("自动命名未运行，无需执行停止操作")
            return
        if self.is_running:
            self.observer.stop()
            self.observer.join()
    ...
    def show_settings_dialog(self):
        ...
        # 启动日志更新定时器
        self.settings_dialog.timer.start(1000)
    ...
    def confirm_settings(self):
        ...
        try:
            ...
            print(f"启动自动命名时出现错误: {e}")
            logging.error(f"启动自动命名时出现错误: {e}")
...
class CustomDialog(QDialog):
    ...
    def update_log_display(self):
        if os.path.exists('auto_naming.log'):
            try:
                with open('auto_naming.log', 'r', encoding='utf-8') as f:
                    log_content = f.read()
                    self.log_textedit.setPlainText(log_content)
            except UnicodeDecodeError:
                self.log_textedit.setPlainText("日志文件编码错误，无法读取")
