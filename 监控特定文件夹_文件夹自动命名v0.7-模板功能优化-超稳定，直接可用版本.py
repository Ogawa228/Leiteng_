...
class TrayApp(QObject):
    ...
    def load_last_settings(self):
        """加载上次的设置"""
        if os.path.exists('last_settings.pkl'):
            with open('last_settings.pkl', 'rb') as f:
                last_settings = pickle.load(f)
                self.target_paths = last_settings.get('target_paths', [])
                self.naming_rules = last_settings.get('naming_rules', {})
                self.include_subfolders = last_settings.get('include_subfolders', {})
    ...
    def save_last_settings(self):
        ...
        with open('last_settings.pkl', 'wb') as f:
            pickle.dump(last_settings, f)
    ...
    def show_settings_dialog(self):
        if self.settings_dialog is None:
            ...
            self.folder_tree.setHeaderLabels(["是否监控", "文件夹名"])
            self.folder_tree.header().setSectionResizeMode(QHeaderView.ResizeToContents)
            self.folder_tree.itemClicked.connect(self.on_folder_item_clicked)
    ...
    def select_folder(self):
        ...
        if folder_path and folder_path not in self.target_paths:  # 避免重复添加
            ...
            self.folder_tree.setItemWidget(item, 0, checkbox)
        ...
        if os.path.isdir(folder_path) and item.childCount() == 0:  # 避免重复加载子文件夹
            for subdir in os.listdir(folder_path):
                ...
                if os.path.isdir(subdir_path):
                    ...
                    self.folder_tree.setItemWidget(child_item, 0, checkbox)
                    # 如果父文件夹被选中，自动选中子文件夹
                    parent_checkbox = self.folder_tree.itemWidget(item, 0)
                    if parent_checkbox.isChecked():
                        checkbox.setChecked(True)

    def on_checkbox_state_changed(self, state, item):
        """当复选框状态改变时，递归处理子文件夹"""
        checkbox = self.folder_tree.itemWidget(item, 0)
        if checkbox.isChecked():
            # 如果选中父文件夹，自动选中所有子文件夹
            for i in range(item.childCount()):
                child_item = item.child(i)
                child_checkbox = self.folder_tree.itemWidget(child_item, 0)
    ...
    def confirm_settings(self):
        ...
        if index == self.naming_rule_combo.count() - 1:
            ...
            if ok:
                for target_path in self.target_paths:
                    self.naming_rules[target_path] = rule
        else:
    ...
    def _process_tree_item(self, item):
        ...
        if checkbox.isChecked():
            ...
            self.target_paths.append(folder_path)
            # 如果文件夹被选中，默认包含所有子文件夹
            self.include_subfolders[folder_path] = True
            # 递归处理子文件夹
            for i in range(item.childCount()):
                child_item = item.child(i)
                self._process_tree_item(child_item)
