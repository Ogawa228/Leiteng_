...
class MyHandler(FileSystemEventHandler):
    def __init__(self, target_path, naming_rule, log_callback, include_subfolders=False, folder_states=None):
...
class TrayApp(QObject):
    ...
    def start_monitoring(self):
        ...
        for target_path in self.target_paths:
            if not os.path.exists(target_path):
                ...
                continue  # 跳过不存在的路径
            event_handler = MyHandler(target_path, self.naming_rules.get(target_path, '{date}-{file_name}'), self.logger.log_message, self.include_subfolders.get(target_path, False), self.folder_states)
    ...
    def apply_wechat_style(self, dialog):
        ...
        palette.setColor(QPalette.WindowText, QColor(77, 77, 77))
    ...
    def handle_naming_rule_selection(self, index):
        ...
        if index == self.naming_rule_combo.count() - 1:
            ...
            if ok and rule:
                self.naming_rule_combo.setItemText(index, rule)  # 更新下拉框中的自定义规则
                self.update_naming_rules(rule)  # 更新命名规则
        else:
            # 预设命名规则
    ...
    def update_naming_rules(self, rule):
        """更新命名规则"""
        for target_path in self.target_paths:
            self.naming_rules[target_path] = rule  # 更新每个目标路径的命名规则
        self.logger.log_message(f"命名规则已更新: {rule}")
