...
class TrayApp(QObject):
    ...
    def show_settings_dialog(self):
        if self.settings_dialog is None:
            ...
            self.folder_tree.setHeaderLabel("监控文件夹")
            self.folder_tree.setColumnCount(2)
            self.folder_tree.setHeaderLabels(["文件夹", "包含子文件夹"])
    ...
    def select_folder(self):
        ...
        if folder_path:
            ...
            checkbox = QCheckBox()
            self.folder_tree.setItemWidget(item, 1, checkbox)
    ...
    def confirm_settings(self):
        ...
        for i in range(self.folder_tree.topLevelItemCount()):
            ...
            target_path = item.text(0)
            checkbox = self.folder_tree.itemWidget(item, 1)
            self.include_subfolders[target_path] = checkbox.isChecked()
    ...
    def new_template(self):
        ...
        if ok and new_rule:
            ...
            if ok and new_path:
                ...
                if file_path:
                    template_data = (os.path.basename(file_path), [new_path], {new_path: new_rule}, {new_path: self.include_subfolders_checkbox.isChecked()})
                    with open(file_path, 'wb') as f:
                        pickle.dump(template_data, f)
                    self.templates.append(template_data)
