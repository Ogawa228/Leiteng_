...
adimi_tool_welcome_message = (3, 0, ti_once, [(multiplayer_is_server),(eq, 0, 1)],#Print Map Informations
...
##  ),
  ...
  (
    ...
    [
      ...
      (62,mtef_visitor_source|mtef_team_1,0,aif_start_alarmed,1,[]),
      (63,mtef_visitor_source|mtef_team_1,0,aif_start_alarmed,1,[]),
     ], adimi_tool_all_gamemodes + cwmm_triggers +
    [
      common_battle_init_banner,
             ...
             (try_begin),
               ...
               (try_begin),
                 ...
                 (try_begin),
                   ...
                   (assign, "$g_winner_team", 1),
                 (try_end),
                 (call_script, "script_cwmm_show_team_score"),
      ...
      (1, 0, 0, [(multiplayer_is_server),
                 ...
                 (val_sub, ":seconds_past_till_round_started", "$g_round_start_time"),
				  ...
				  (le, ":remaining_time", CWMM_DEATH_MODE_TIME),
      ...
      (1, 0, 0, [(multiplayer_is_server),
       ...
       [
         ...
         (try_begin),
             (lt, "$g_cwmm_last_round_entry_point", 0),
             (store_random_in_range, "$g_random_entry_point", 0, 3),
         (else_try),
             (store_random_in_range, "$g_random_entry_point", 1, 3),
             (val_add, "$g_random_entry_point", "$g_cwmm_last_round_entry_point"),
             (val_mod, "$g_random_entry_point", 3),
         (try_end),
         (assign, "$g_cwmm_last_round_entry_point", "$g_random_entry_point"),
         ...
         (try_for_range, ":player_no", 0, ":num_players"),
           ...
           (try_begin),
             ...
             (try_begin),
               ...
               (try_begin),
					(call_script,"script_adimi_tool_get_troop_class",":player_troop"),
					(assign,":troop_class",reg0),
					(try_begin),
						(eq,":troop_class",multi_troop_class_cavalry),
						(assign,":cav_timer","$g_round_start_time"),
						(val_sub, ":cav_timer",6),
						(le,":round_time",":cav_timer"),
						(eq, "$g_round_ended", 0),
						(assign, ":spawn_new", 1),
					(else_try),
						(eq, "$g_round_ended", 0),
             ...
             (try_end),
             (call_script, "script_multiplayer_buy_agent_equipment", ":player_no"),
             (call_script, "script_cwmm_event_player_spawn_as_troop_class", ":player_no", ":troop_class"),
             (player_spawn_new_agent, ":player_no", ":entry_no"),
             (player_set_slot, ":player_no", slot_player_spawned_this_round, 1),
