...
# 为拍卖列表表的F列填充辅助列
def fill_auxiliary_column(ws_auction):
    for row in range(12, 145):
        ws_auction.cell(row=row, column=6).value = f"=D{row} & \"\""

fill_auxiliary_column(auction_sheet)

# 为队伍总览表中的各队伍区域填充公式
def fill_team_overview_formulas(ws_overview):
    team_ranges = {
        "【CbHz】吃饱喝足就是玩": ("B5", "D33"),
        "【SSR】Superior Super Rare": ("G5", "I33"),
        "【OldH】老H犟嘴": ("L5", "N33"),
        "【AWO】逃出'生天": ("Q5", "S33"),
        "【G2】G2": ("B37", "D61"),
        "【TV】Team Vitality": ("G37", "I61"),
        "【Miniworld】迷你世界": ("L37", "N61"),
        "【NikoFC】NikoCN Fanclub": ("Q37", "S61")
    }

    for team, (start_cell, end_cell) in team_ranges.items():
        start_row = int(start_cell[1:])
        end_row = int(end_cell[1:])
        
        for row in range(start_row, end_row + 1):
            ws_overview[f"B{row}"].value = f"=IFERROR(FILTER(拍卖列表!$A$12:$A$144, 拍卖列表!$F$12:$F$144=\"{team}\"), \"\")"
            ws_overview[f"C{row}"].value = f"=IFERROR(FILTER(拍卖列表!$B$12:$B$144, 拍卖列表!$F$12:$F$144=\"{team}\"), \"\")"
            ws_overview[f"D{row}"].value = f"=IFERROR(FILTER(拍卖列表!$C$12:$C$144, 拍卖列表!$F$12:$F$144=\"{team}\"), \"\")"

fill_team_overview_formulas(team_overview_sheet)
