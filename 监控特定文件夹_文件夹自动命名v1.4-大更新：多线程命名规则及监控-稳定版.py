...
class TrayApp(QObject):
    ...
    def on_naming_rule_changed(self, index, item):
        ...
        if index == combo_box.count() - 1:
            ...
            if ok and rule:
                combo_box.setItemText(index, rule)  # 更新下拉框中的自定义规则
    ...
    def save_current_settings_as_template(self):
        ...
        if file_path:
            template_data = (os.path.basename(file_path), self.target_paths, self.naming_rules, self.include_subfolders, self.folder_states)
    ...
    def save_template(self):
        ...
        if file_path:
            ...
            template_data = (os.path.basename(self.template_path), self.target_paths, self.naming_rules, self.include_subfolders, self.folder_states)
    ...
    def new_template(self):
        ...
        if ok and new_rule:
            ...
            if ok and new_path:
                ...
                if file_path:
                    template_data = (os.path.basename(file_path), [new_path], {new_path: new_rule}, {new_path: True}, {new_path: True})
                    with open(file_path, 'wb') as f:
                        pickle.dump(template_data, f)
                    self.templates.append(template_data)
