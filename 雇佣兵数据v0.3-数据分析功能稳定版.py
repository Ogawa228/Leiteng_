...
class ExcelSaver:
        ...
        if ui.save_path:
            try:
                ...
                for match_index, match in enumerate(matches):
                    ...
                    worksheet.insert_image(f'A{len(df) + 2}', chart_path, {'x_scale': 0.6, 'y_scale': 0.6})

                if ui.excel_writer:
                    ui.excel_writer.close()
    ...
    def prepare_match_dataframe(match, ui):
        ...
        for player, hits in hit_data.items():
            ...
            rank_score = 1 * (0.485 *( 0.8 * ((combo_hits_ratio + 2 * k_dmg_damage_ratio) / 2) * 100
                          + 1.1 * (k_dmg_combo_ratio - received_k_dmg_combo_ratio)
                          + 2.5 * combo_count
                          + 0.01 * k_dmg
                          + 1.5 * len(hits)) + kill_death_ratio * 2 - tk_kills * 5)
