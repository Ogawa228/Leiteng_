...
class ADBControlApp(QWidget):
    def __init__(self):
        ...
        self.timers = {}
        self.timer_to_task = {}
                # 在初始化ADBCommandExecutor时传递界面切换的方法引用
        self.adb_command_executor = ADBCommandExecutor(
            "device_id", "package_name", "activity_name", 
            back_to_main_ui=self.back_to_main_ui, back_to_config_ui=self.back_to_config_ui
        )
        self.init_ui()
        self.command_log = []
    ...
    def init_ui(self):
        ...
        # 切换到命令配置界面的按钮
        to_commands_ui_btn = QPushButton('ADB命令配置')
        to_commands_ui_btn.clicked.connect(lambda: self.stack_widget.setCurrentIndex(2))  # 假设索引2为命令配置界面
        config_layout.addWidget(to_commands_ui_btn)
        ...
        # 获取命令配置UI
        commands_ui = self.adb_command_executor.create_commands_ui()
        self.stack_widget.addWidget(commands_ui)
        ...
        self.apply_apple_style(to_commands_ui_btn)
