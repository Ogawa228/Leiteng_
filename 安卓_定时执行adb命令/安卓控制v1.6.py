...
import os
import subprocess
from PyQt5.QtWidgets import (QDialog, QVBoxLayout, QLabel, QLineEdit, QCheckBox, QPushButton, QDialogButtonBox, QMessageBox, QProgressBar)
...
class ADBParametersDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.resize(800, int(800 / 1.618))  # 设置窗口大小为黄金比例
    ...
    def init_ui(self):
        ...
        self.instruction_label = QLabel("以下是ADB的当前配置参数，如果不正确，请修改：")
        layout.addWidget(self.instruction_label)

        # 将默认参数填写到文本编辑框中
        self.parameters_text_edit = QTextEdit(self)
        self.parameters_text_edit.setText(f"设备ID: {self.device_id}\n包名: {self.package_name}\n活动名: {self.activity_name}")
        layout.addWidget(self.parameters_text_edit)

        # 添加复选框以允许用户选择是否在下次启动时自动检测ADB参数
        self.auto_detect_checkbox = QCheckBox("自动检测ADB参数（下次不再使用此选项）")
        self.auto_detect_checkbox.setChecked(True)
        layout.addWidget(self.auto_detect_checkbox)

        # 添加获取ADB参数指引的按钮
        self.adb_guide_button = QPushButton("如何获取ADB参数指引")
        self.adb_guide_button.clicked.connect(self.show_adb_guide)
        layout.addWidget(self.adb_guide_button)

        # 添加复选框以允许用户选择是否在下次启动时跳过此对话框
        self.skip_checkbox = QCheckBox("下次不再显示此对话框")
    ...
    def show_adb_guide(self):
        ...
        QMessageBox.information(self, "ADB参数获取指引", guide_text)

    def get_parameters(self):
        # 从文本编辑框中读取参数并分割成列表
        parameters_text = self.parameters_text_edit.toPlainText()
        parameters_list = [line.split(": ")[1] for line in parameters_text.split("\n") if ": " in line]
        return parameters_list, self.skip_checkbox.isChecked(), self.auto_detect_checkbox.isChecked()
...
class ADBControlApp(QWidget):
    ...
    def show_message(self):
        ...
        # 设置 QTimer 来关闭消息框
        QTimer.singleShot(5000, msg.close) # 5秒后自动关闭, 你可以根据需要设置不同的时间
        msg.exec_()    
        # 新增：消息框关闭后，调用程序退出
    ...
    def show_adb_parameters_dialog(self):
        ...
        if adb_parameters_dialog.exec_() == QDialog.Accepted:
            # 更新此处以接收三个返回值
            params, do_not_show, auto_detect = adb_parameters_dialog.get_parameters()
            ...
            # 可选：处理自动检测ADB参数的复选框状态
            # 如果 auto_detect 为 True，则执行相关的自动检测逻辑
            if auto_detect:
                # 此处添加自动检测ADB参数的代码
                pass
...
def main():
    ...
    if dialog.exec_():
        device_id = dialog.device_id_edit.text()
        package_name = dialog.package_name_edit.text()
        activity_name = dialog.activity_name_edit.text()
        print(f"Device ID: {device_id}, Package Name: {package_name}, Activity Name: {activity_name}")
    sys.exit(app.exec_())
    ex = ADBControlApp()
    ex.show()
    sys.exit(app.exec_())
