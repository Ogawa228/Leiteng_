import sys
import time
import os
import pickle
import winreg
import re
import logging
import ctypes
from watchdog.observers import Observer
from watchdog.events import FileSystemEventHandler
from PyQt5.QtWidgets import (
    QApplication, QSystemTrayIcon, QMenu, QMessageBox, QInputDialog,
    QFileDialog, QComboBox, QVBoxLayout, QDialog, QPushButton, QLabel,
    QCheckBox, QSizePolicy, QWidget, QHBoxLayout, QTextEdit, QTreeWidget,
    QTreeWidgetItem, QHeaderView, QLineEdit, QTableWidget, QTableWidgetItem,
    QFrame, QGroupBox, QFormLayout, QDateTimeEdit
)
from PyQt5.QtGui import QIcon, QColor, QPalette, QFont
from PyQt5.QtCore import (
    Qt, QTimer, pyqtSignal, QObject, QDateTime
)
from datetime import datetime
from concurrent.futures import ThreadPoolExecutor

# 提升程序权限为管理员
def run_as_admin():
    if ctypes.windll.shell32.IsUserAnAdmin():
        return True
    else:
        ctypes.windll.shell32.ShellExecuteW(None, "runas", sys.executable, " ".join(sys.argv), None, 1)
        sys.exit()

# 初始化日志记录
logging.basicConfig(filename='auto_naming.log', level=logging.WARNING,  # 设置日志级别为 WARNING
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    encoding='utf-8')  # 添加 encoding='utf-8'

# 用于清理文件名中的非法字符
def clean_filename(filename):
    return re.sub(r'[<>:"/\\|?*]', '', filename)

class Logger:
    def __init__(self, log_file='log_history.txt'):
        self.log_file = log_file
        self.log_history = []
        self.load_log_history()

    def log_message(self, message):
        """记录日志到全局日志列表"""
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        log_entry = f"{timestamp} - {message}"
        
        # 检查日志内容是否已经存在，避免重复记录
        if log_entry not in self.log_history:
            self.log_history.append(log_entry)
            self.save_log_history(log_entry)  # 只追加新日志
            return log_entry
        return None

    def load_log_history(self):
        """从文件加载日志历史"""
        if os.path.exists(self.log_file):
            with open(self.log_file, 'r', encoding='utf-8') as f:
                self.log_history = f.read().splitlines()

    def save_log_history(self, log_entry):
        """保存日志历史到文件（只追加新日志）"""
        with open(self.log_file, 'a', encoding='utf-8') as f:  # 使用追加模式
            f.write(log_entry + "\n")  # 只追加新日志内容

    def query_logs(self, start_time, end_time):
        """查询日志"""
        logs = []
        for log_entry in self.log_history:
            parts = log_entry.split(' - ')
            if len(parts) >= 2:
                timestamp, message = parts[0], parts[1]
                if start_time <= timestamp <= end_time:
                    logs.append((timestamp, message))
        return logs

class MyHandler(FileSystemEventHandler):
    def __init__(self, target_path, naming_rules, log_callback, include_subfolders=False, folder_states=None):
        self.target_path = target_path
        self.naming_rules = naming_rules
        self.log_callback = log_callback
        self.include_subfolders = include_subfolders
        self.folder_states = folder_states or {}  # 保存文件夹的选中状态
        self.file_counter = 1  # 用于顺序命名
        super().__init__()

    def is_temp_or_hidden_file(self, file_path):
        """判断是否为临时文件或隐藏文件"""
        file_name = os.path.basename(file_path)
        # 判断是否为隐藏文件（以.开头或具有隐藏属性）
        if file_name.startswith('.') or (os.name == 'nt' and os.stat(file_path).st_file_attributes & 0x2):
            return True
        # 判断是否为临时文件（常见的临时文件后缀）
        temp_extensions = ['.tmp', '.temp', '~']
        if any(file_name.lower().endswith(ext) for ext in temp_extensions):
            return True
        return False

    def is_file_from_other_folder(self, file_path):
        """判断文件是否是从其他文件夹复制或移动过来的"""
        # 获取文件的绝对路径
        file_abs_path = os.path.abspath(file_path)
        # 获取文件的父文件夹路径
        file_parent_dir = os.path.dirname(file_abs_path)
        # 如果文件的父文件夹路径与目标路径相同，则认为是非外部文件
        if file_parent_dir == self.target_path:
            return False
        # 放宽判断条件，只要文件不在目标路径中，就认为是外部文件
        return True

    def is_folder_selected(self, file_path):
        """检查文件所在的文件夹是否被选中"""
        for folder_path, is_selected in self.folder_states.items():
            if file_path.startswith(folder_path):
                return is_selected
        return True  # 默认返回 True，如果文件夹状态未保存

    def on_created(self, event):
        if not event.is_directory:
            file_path = event.src_path
            self.log_callback(f"检测到新文件: {file_path}")  # 记录文件创建事件

            # 忽略临时文件或隐藏文件
            if self.is_temp_or_hidden_file(file_path):
                self.log_callback(f'忽略临时文件或隐藏文件: {file_path}')
                return

            # 判断文件是否是从其他文件夹复制或移动过来的
            if not self.is_file_from_other_folder(file_path):
                self.log_callback(f'忽略非外部文件: {file_path}')
                return

            # 检查文件所在的文件夹是否被选中
            if not self.is_folder_selected(file_path):
                self.log_callback(f'忽略未选中的文件夹中的文件: {file_path}')
                return

            # 延迟一段时间，确保文件已经完全写入磁盘
            time.sleep(1)  # 延迟 1 秒

            # 获取文件的基本信息
            file_dir, file_name = os.path.split(file_path)
            file_name_no_ext, file_ext = os.path.splitext(file_name)
            file_name_no_ext = clean_filename(file_name_no_ext)

            # 获取当前日期，并去掉月份和天数的前导零
            now = datetime.now()
            current_date = f"{now.year}-{now.month}-{now.day}"

            # 获取文件夹的命名规则
            naming_rule = self.naming_rules.get(file_dir, '{date}-{file_name}')

            # 根据命名规则生成新文件名
            if naming_rule == "顺序命名":
                new_file_name = f"{self.file_counter}.{file_name_no_ext}{file_ext}"
                self.file_counter += 1
            else:
                new_file_name = naming_rule.format(file_name=file_name_no_ext, date=current_date) + file_ext

            # 生成新文件路径
            new_file_path = os.path.join(file_dir, new_file_name)

            # 处理文件名冲突
            counter = 1
            while os.path.exists(new_file_path):
                new_file_name = f"{file_name_no_ext}_{counter}{file_ext}"
                new_file_path = os.path.join(file_dir, new_file_name)
                counter += 1

            # 重命名文件，增加重试机制
            max_retries = 5  # 最大重试次数
            retry_delay = 1  # 每次重试的延迟时间（秒）
            for retry in range(max_retries):
                try:
                    os.rename(file_path, new_file_path)
                    self.log_callback(f'重命名文件 {file_path} 为 {new_file_path} 成功')
                    break
                except OSError as e:
                    if retry == max_retries - 1:
                        self.log_callback(f'重命名文件 {file_path} 失败，经过 {max_retries} 次重试: {e}')
                    time.sleep(retry_delay)  # 延迟一段时间后重试

class DynamicMonitor(QObject):
    def __init__(self, parent):
        super().__init__(parent)
        self.parent = parent
        self.timer = QTimer()
        self.timer.timeout.connect(self.check_for_changes)
        self.last_naming_rules = None
        self.last_target_paths = None
        self.last_folder_states = None

    def start(self):
        """启动动态监控"""
        self.last_naming_rules = self.parent.naming_rules.copy()
        self.last_target_paths = self.parent.target_paths.copy()
        self.last_folder_states = self.parent.folder_states.copy()
        self.timer.start(1000)  # 每1秒检查一次

    def stop(self):
        """停止动态监控"""
        self.timer.stop()

    def check_for_changes(self):
        """检查命名规则和目录树是否发生变化"""
        if (self.last_naming_rules != self.parent.naming_rules or
            self.last_target_paths != self.parent.target_paths or
            self.last_folder_states != self.parent.folder_states):
            self.parent.logger.log_message("检测到命名规则或目录树变化，重新启动监控")
            self.parent.stop_auto_naming()
            self.parent.start_auto_naming()
            # 更新缓存
            self.last_naming_rules = self.parent.naming_rules.copy()
            self.last_target_paths = self.parent.target_paths.copy()
            self.last_folder_states = self.parent.folder_states.copy()

class TrayApp(QObject):
    monitor_started = pyqtSignal()
    log_updated = pyqtSignal(str)  # 新增信号，用于传递日志内容

    def __init__(self):
        super().__init__()
        self.app = QApplication(sys.argv)
        self.app.setQuitOnLastWindowClosed(False)  # 确保程序不因最后一个窗口关闭而退出
        self.app.setApplicationName("工作流自动化-文件重命名")  # 设置程序名称

        # 创建一个主窗口，用于绑定快捷键
        self.main_window = QWidget()
        self.main_window.setWindowTitle("工作流自动化-文件重命名")
        self.main_window.hide()  # 隐藏主窗口，只显示托盘图标

        self.icon_path = r"C:\Users\Administrator\OneDrive\Desktop\py\屏幕截图 2024-01-16 000502-1种尺寸.ico"
        self.target_paths = []  # 多个目标路径
        self.naming_rules = {}  # 每个路径对应的命名规则
        self.include_subfolders = {}  # 每个路径是否包含子文件夹
        self.folder_states = {}  # 每个文件夹及其子文件夹的选中状态
        self.is_running = False
        self.template_path = ""
        self.templates = []
        self.custom_naming_rules = []  # 保存自定义命名规则
        self.startup_checkbox = QCheckBox('开机自动启动')
        self.logger = Logger()  # 使用Logger类管理日志
        self.load_templates()
        self.load_startup_status()
        self.load_last_settings()  # 加载上次的设置

        # 初始化动态监控模块
        self.dynamic_monitor = DynamicMonitor(self)

        self.monitor_started.connect(self.show_start_monitor_notification)

        # 创建系统托盘图标
        self.tray = QSystemTrayIcon(QIcon(self.icon_path))
        self.tray.activated.connect(self.on_tray_activated)
        self.tray.show()

        # 创建菜单
        self.menu = QMenu()
        self.start_action = self.menu.addAction('开始自动命名')
        self.stop_action = self.menu.addAction('结束自动命名')
        self.settings_action = self.menu.addAction('设置')
        self.template_action = self.menu.addAction('模板')
        self.exit_action = self.menu.addAction('关闭程序')

        self.start_action.triggered.connect(self.start_auto_naming)
        self.stop_action.triggered.connect(self.stop_auto_naming)
        self.settings_action.triggered.connect(self.show_settings_dialog)
        self.template_action.triggered.connect(self.show_template_menu)
        self.exit_action.triggered.connect(self.exit_app)

        self.tray.setContextMenu(self.menu)

        # 初始化观察者
        self.observer = None
        self.executor = ThreadPoolExecutor()

        # 初始化 settings_dialog 为 None
        self.settings_dialog = None

        # 程序启动后自动启动监控
        self.start_auto_naming()

    def load_last_settings(self):
        """加载上次的设置"""
        if os.path.exists('last_settings.template'):
            try:
                with open('last_settings.template', 'rb') as f:
                    template_data = pickle.load(f)
                    self.target_paths = template_data.get('target_paths', [])
                    self.naming_rules = template_data.get('naming_rules', {})
                    self.include_subfolders = template_data.get('include_subfolders', {})
                    self.folder_states = template_data.get('folder_states', {})  # 加载文件夹状态
                    self.custom_naming_rules = template_data.get('custom_naming_rules', [])  # 加载自定义命名规则
                    self.logger.log_message("成功加载缓存模板: last_settings.template")
                    # 如果上次监控是开启状态，则自动启动监控
                    if template_data.get('is_running', False):
                        self.start_auto_naming()
            except Exception as e:
                self.logger.log_message(f"加载缓存模板时出现错误: {e}")
        else:
            self.logger.log_message("未找到缓存模板，使用默认设置")

    def update_ui_with_last_settings(self):
        """将加载的设置反馈到界面"""
        if self.settings_dialog is not None:
            # 清空目录树
            self.settings_dialog.folder_tree.clear()
            # 添加文件夹到目录树
            for target_path in self.target_paths:
                item = QTreeWidgetItem(self.settings_dialog.folder_tree)
                item.setText(1, target_path)  # 文件夹名
                checkbox = QCheckBox()
                checkbox.setChecked(self.folder_states.get(target_path, True))  # 根据保存的状态设置复选框
                checkbox.stateChanged.connect(lambda state, item=item: self.on_checkbox_state_changed(state, item))
                self.settings_dialog.folder_tree.setItemWidget(item, 0, checkbox)
                
                # 添加命名规则下拉菜单
                naming_rule_combo = QComboBox()
                naming_rule_combo.addItems(['{date}-{file_name}', '{file_name}-{date}', '{date}_{file_name}', '顺序命名'])
                naming_rule_combo.addItems(self.custom_naming_rules)  # 添加自定义命名规则
                naming_rule_combo.addItem('自定义')  # 始终将“自定义”放在最后
                naming_rule_combo.setCurrentText(self.naming_rules.get(target_path, '{date}-{file_name}'))
                naming_rule_combo.currentIndexChanged.connect(lambda index, item=item: self.on_naming_rule_changed(index, item))
                self.settings_dialog.folder_tree.setItemWidget(item, 2, naming_rule_combo)
                
                # 添加删除按钮
                delete_button = QPushButton("❎")
                delete_button.setStyleSheet("""
                    QPushButton {
                        background-color: transparent;
                        border: none;
                        font-size: 14px;
                    }
                    QPushButton:hover {
                        color: #ff0000;
                    }
                    QPushButton:pressed {
                        color: #cc0000;
                    }
                """)
                delete_button.clicked.connect(lambda: self.delete_folder_item(item))
                self.settings_dialog.folder_tree.setItemWidget(item, 3, delete_button)

                # 加载子文件夹的选中状态
                self.load_subfolder_states(item, target_path)

    def load_subfolder_states(self, parent_item, parent_path):
        """加载子文件夹的选中状态"""
        for subdir in os.listdir(parent_path):
            subdir_path = os.path.join(parent_path, subdir)
            if os.path.isdir(subdir_path):
                child_item = QTreeWidgetItem(parent_item)
                child_item.setText(1, subdir_path)  # 文件夹名
                checkbox = QCheckBox()
                checkbox.setChecked(self.folder_states.get(subdir_path, True))  # 根据保存的状态设置复选框
                checkbox.stateChanged.connect(lambda state, item=child_item: self.on_checkbox_state_changed(state, item))
                self.settings_dialog.folder_tree.setItemWidget(child_item, 0, checkbox)
                
                # 添加命名规则下拉菜单
                naming_rule_combo = QComboBox()
                naming_rule_combo.addItems(['{date}-{file_name}', '{file_name}-{date}', '{date}_{file_name}', '顺序命名'])
                naming_rule_combo.addItems(self.custom_naming_rules)  # 添加自定义命名规则
                naming_rule_combo.addItem('自定义')  # 始终将“自定义”放在最后
                naming_rule_combo.setCurrentText(self.naming_rules.get(subdir_path, '{date}-{file_name}'))
                naming_rule_combo.currentIndexChanged.connect(lambda index, item=child_item: self.on_naming_rule_changed(index, item))
                self.settings_dialog.folder_tree.setItemWidget(child_item, 2, naming_rule_combo)
                
                # 添加删除按钮
                delete_button = QPushButton("❎")
                delete_button.setStyleSheet("""
                    QPushButton {
                        background-color: transparent;
                        border: none;
                        font-size: 14px;
                    }
                    QPushButton:hover {
                        color: #ff0000;
                    }
                    QPushButton:pressed {
                        color: #cc0000;
                    }
                """)
                delete_button.clicked.connect(lambda: self.delete_folder_item(child_item))
                self.settings_dialog.folder_tree.setItemWidget(child_item, 3, delete_button)

                # 递归加载子文件夹
                self.load_subfolder_states(child_item, subdir_path)

    def save_last_settings(self):
        """保存当前设置为缓存模板"""
        template_data = {
            'target_paths': self.target_paths,
            'naming_rules': self.naming_rules,
            'include_subfolders': self.include_subfolders,
            'folder_states': self.folder_states,  # 保存文件夹状态
            'is_running': self.is_running,  # 保存监控状态
            'custom_naming_rules': self.custom_naming_rules  # 保存自定义命名规则
        }
        try:
            with open('last_settings.template', 'wb') as f:
                pickle.dump(template_data, f)
            self.logger.log_message("成功保存缓存模板: last_settings.template")
        except Exception as e:
            self.logger.log_message(f"保存缓存模板时出现错误: {e}")

    def is_startup_enabled(self):
        """检查当前是否启用了开机自启"""
        try:
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_READ)
            try:
                value, _ = winreg.QueryValueEx(key, "工作流自动化-文件重命名")
                return value is not None
            finally:
                winreg.CloseKey(key)
        except FileNotFoundError:
            return False
        except Exception as e:
            self.logger.log_message(f"检查开机自启状态时出现错误: {e}")
            return False

    def load_startup_status(self):
        """加载开机自启状态"""
        try:
            # 打开注册表键
            key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_READ)
            try:
                # 查询注册表值
                value, _ = winreg.QueryValueEx(key, "工作流自动化-文件重命名")
                if value:
                    self.startup_checkbox.setChecked(True)
                    self.logger.log_message("开机自启状态: 已启用")
                else:
                    # 注册表值存在但为空，认为是未启用
                    self.startup_checkbox.setChecked(False)
                    self.logger.log_message("开机自启状态: 未启用（注册表值存在但为空）")
            finally:
                # 关闭注册表键
                winreg.CloseKey(key)
        except FileNotFoundError:
            # 注册表路径或键值不存在，认为是未启用
            self.startup_checkbox.setChecked(False)
            self.logger.log_message("开机自启状态: 未启用（注册表路径或键值不存在）")
        except Exception as e:
            # 处理其他异常情况
            self.startup_checkbox.setChecked(False)
            self.logger.log_message(f"加载开机自启状态时出现错误: {e}")

    def set_startup(self, enable):
        """设置开机自启"""
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0,
                             winreg.KEY_SET_VALUE)
        if enable:
            script_path = os.path.abspath(sys.argv[0])
            winreg.SetValueEx(key, "工作流自动化-文件重命名", 0, winreg.REG_SZ, script_path)
            self.logger.log_message("已启用开机自启")
            # 开机自动启动监控
            self.start_auto_naming()
        else:
            try:
                winreg.DeleteValue(key, "工作流自动化-文件重命名")
                self.logger.log_message("已禁用开机自启")
            except FileNotFoundError:
                pass
        winreg.CloseKey(key)

    def start_auto_naming(self):
        if self.is_running:
            self.logger.log_message("自动命名已经在运行中，无需重复启动")
            return
        if not self.is_running:
            self.is_running = True
            self.executor = ThreadPoolExecutor()  # 重新初始化executor
            self.start_monitoring()  # 启动监控
            self.dynamic_monitor.start()  # 启动动态监控
            self.monitor_started.emit()
            self.save_last_settings()  # 启动监控后自动保存缓存模板

    def start_monitoring(self):
        """启动监控"""
        if self.observer:
            self.observer.stop()  # 停止当前的Observer
            if self.observer.is_alive():  # 检查线程是否正在运行
                self.observer.join()  # 等待线程结束
        self.observer = Observer()
        for target_path in self.target_paths:
            if not os.path.exists(target_path):
                self.logger.log_message(f"目标路径不存在: {target_path}")
                continue  # 跳过不存在的路径
            event_handler = MyHandler(target_path, self.naming_rules, self.logger.log_message, self.include_subfolders.get(target_path, False), self.folder_states)
            self.observer.schedule(event_handler, target_path, recursive=True)  # 递归监控所有子目录
        self.executor.submit(self.observer.start)
        self.logger.log_message("监控已启动")

    def stop_auto_naming(self):
        if not self.is_running:
            self.logger.log_message("自动命名未运行，无需执行停止操作")
            return
        if self.is_running:
            self.is_running = False
            self.executor.shutdown(wait=False)
            self.dynamic_monitor.stop()  # 停止动态监控
            self.logger.log_message("监控已停止")
            self.save_last_settings()  # 停止监控后自动保存缓存模板
            # 使用系统托盘通知
            self.tray.showMessage(
                "监控已停止",
                "文件夹监控已停止，自动命名功能已禁用。",
                QSystemTrayIcon.Information,  # 通知类型
                1000  # 通知显示时间（毫秒）
            )

    def on_tray_activated(self, reason):
        if reason == QSystemTrayIcon.Trigger:  # 单击
            self.show_settings_dialog()
        elif reason == QSystemTrayIcon.DoubleClick:  # 双击
            self.show_settings_dialog()

    def show_settings_dialog(self):
        if self.settings_dialog is None:
            self.settings_dialog = CustomDialog(self)  # 将 self 作为 parent_widget 传递
            self.apply_wechat_style(self.settings_dialog)
            self.settings_dialog.setWindowTitle('设置')
            self.settings_dialog.resize(1000, 800)  # 设置对话框宽度

            # 设置全局字体
            font = QFont("Roboto", 12)
            font.setBold(True)
            self.settings_dialog.setFont(font)

            # 主布局
            main_layout = QVBoxLayout()

            # 文件夹监控模块
            folder_group = QGroupBox("文件夹监控")
            folder_layout = QVBoxLayout()
            folder_label = QLabel('')
            self.folder_button = QPushButton('增加文件夹')
            self.folder_button.clicked.connect(self.select_folder)
            self.settings_dialog.folder_tree = QTreeWidget()
            self.settings_dialog.folder_tree.setHeaderLabels(["是否监控", "文件夹名", "命名规则", "删除"])
            self.settings_dialog.folder_tree.header().setSectionResizeMode(QHeaderView.ResizeToContents)
            self.settings_dialog.folder_tree.setStyleSheet("""
                QTreeWidget {
                    border: 1px solid #e0e0e0;
                    border-radius: 4px;
                    background-color: white;
                    font-family: "Roboto", sans-serif;
                    font-size: 12px;
                    padding: 5px;
                }
                QTreeWidget::item {
                    height: 36px;
                    padding: 5px;
                    border-bottom: 1px solid #eee;
                }
                QTreeWidget::item:hover {
                    background-color: #f5f5f5;
                }
                QTreeWidget::item:selected {
                    background-color: #e3f2fd;
                    color: #1976d2;
                }
                QHeaderView::section {
                    background-color: #f5f5f5;
                    padding: 8px;
                    border: none;
                    font-weight: 500;
                    font-size: 12px;
                    color: #757575;
                }
            """)
            self.settings_dialog.folder_tree.itemClicked.connect(self.on_folder_item_clicked)

            folder_layout.addWidget(folder_label)
            folder_layout.addWidget(self.folder_button)
            folder_layout.addWidget(self.settings_dialog.folder_tree)
            folder_group.setLayout(folder_layout)

            # 日志功能模块
            log_group = QGroupBox("日志记录")
            log_layout = QVBoxLayout()
            log_label = QLabel('')
            self.log_table = QTableWidget()
            self.log_table.setColumnCount(2)
            self.log_table.setHorizontalHeaderLabels(['操作时间', '操作内容'])
            self.log_table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

            # 日志查询功能
            log_query_layout = QHBoxLayout()
            self.log_query_button = QPushButton('查询日志')
            self.log_query_button.clicked.connect(self.show_log_query_dialog)

            log_query_layout.addWidget(self.log_query_button)

            log_layout.addWidget(log_label)
            log_layout.addLayout(log_query_layout)
            log_layout.addWidget(self.log_table)
            log_group.setLayout(log_layout)

            # 开机自启动功能
            startup_group = QGroupBox("")
            startup_layout = QVBoxLayout()
            self.startup_checkbox = QCheckBox('开机自动启动')
            self.startup_checkbox.stateChanged.connect(self.toggle_startup)
            startup_layout.addWidget(self.startup_checkbox)
            startup_group.setLayout(startup_layout)

            # 新增功能区
            action_group = QGroupBox("")
            action_layout = QVBoxLayout()

            # 第一行按钮
            first_row_layout = QHBoxLayout()
            self.save_template_button = QPushButton('保存为模板')
            self.load_template_button = QPushButton('加载模板')
            first_row_layout.addWidget(self.save_template_button)
            first_row_layout.addWidget(self.load_template_button)

            # 第二行按钮
            second_row_layout = QHBoxLayout()
            self.start_monitor_button = QPushButton('启动监控')
            self.stop_monitor_button = QPushButton('停止监控')
            second_row_layout.addWidget(self.start_monitor_button)
            second_row_layout.addWidget(self.stop_monitor_button)

            action_layout.addLayout(first_row_layout)
            action_layout.addLayout(second_row_layout)
            action_group.setLayout(action_layout)

            # 主布局添加模块
            main_layout.addWidget(folder_group)
            main_layout.addWidget(log_group)
            main_layout.addWidget(startup_group)
            main_layout.addWidget(action_group)

            self.settings_dialog.setLayout(main_layout)
            self.settings_dialog.closeEvent = self.handle_dialog_close

            # 连接日志更新信号
            self.log_updated.connect(self.settings_dialog.update_log_display)

            # 设置定时器，定期刷新日志显示
            self.log_timer = QTimer()
            self.log_timer.timeout.connect(self.settings_dialog.load_log_history)
            self.log_timer.start(1000)  # 每1秒刷新一次

            # 连接底部按钮的点击事件
            self.save_template_button.clicked.connect(self.save_current_settings_as_template)
            self.load_template_button.clicked.connect(self.load_template)
            self.start_monitor_button.clicked.connect(self.start_auto_naming)
            self.stop_monitor_button.clicked.connect(self.stop_auto_naming)

        # 确保开机自启状态同步到 UI
        self.startup_checkbox.setChecked(self.is_startup_enabled())  # 同步复选框状态
        # 更新日志显示
        self.settings_dialog.load_log_history()
        # 如果 settings_dialog 已经初始化，更新界面
        if self.settings_dialog is not None:
            self.update_ui_with_last_settings()
        self.settings_dialog.show()

    def toggle_startup(self, state):
        """切换开机自启状态"""
        if state == Qt.Checked:
            self.set_startup(True)
        else:
            self.set_startup(False)

    def show_log_query_dialog(self):
        """显示日志查询对话框"""
        dialog = QDialog(self.settings_dialog)
        dialog.setWindowTitle('日志查询')
        dialog.resize(400, 200)

        layout = QVBoxLayout()

        # 开始时间选择
        start_time_label = QLabel('开始时间:')
        self.start_time_edit = QDateTimeEdit()
        self.start_time_edit.setDateTime(QDateTime.currentDateTime())
        layout.addWidget(start_time_label)
        layout.addWidget(self.start_time_edit)

        # 结束时间选择
        end_time_label = QLabel('结束时间:')
        self.end_time_edit = QDateTimeEdit()
        self.end_time_edit.setDateTime(QDateTime.currentDateTime())
        layout.addWidget(end_time_label)
        layout.addWidget(self.end_time_edit)

        # 查询按钮
        query_button = QPushButton('查询')
        query_button.clicked.connect(lambda: self.query_logs(
            self.start_time_edit.dateTime().toString('yyyy-MM-dd HH:mm:ss'),
            self.end_time_edit.dateTime().toString('yyyy-MM-dd HH:mm:ss')
        ))
        layout.addWidget(query_button)

        dialog.setLayout(layout)
        dialog.exec_()

    def query_logs(self, start_time, end_time):
        """查询日志"""
        self.log_table.setRowCount(0)
        logs = self.logger.query_logs(start_time, end_time)
        for timestamp, message in logs:
            row = self.log_table.rowCount()
            self.log_table.insertRow(row)
            self.log_table.setItem(row, 0, QTableWidgetItem(timestamp))
            self.log_table.setItem(row, 1, QTableWidgetItem(message))

    def handle_dialog_close(self, event):
        self.settings_dialog.hide()
        event.ignore()

    def apply_wechat_style(self, dialog):
        # 设置整体背景色为浅灰色
        palette = dialog.palette()
        palette.setColor(QPalette.Window, QColor(250, 250, 250))
        palette.setColor(QPalette.WindowText, QColor(33, 33, 33))
        dialog.setPalette(palette)

        # 对按钮应用 Material Design 风格
        style_sheet = """
            QPushButton {
                background-color: #1976d2;
                border: none;
                border-radius: 4px;
                padding: 8px 16px;
                color: #333333;
                font-family: "Roboto", sans-serif;
                font-size: 12px;
                font-weight: 500;
                min-width: 80px;
            }
            QPushButton:hover {
                background-color: #1565c0;
            }
            QPushButton:pressed {
                background-color: #0d47a1;
            }
            QLabel {
                color: #333;
                font-family: "Roboto", sans-serif;
                font-size: 12px;
                font-weight: 500;
            }
            QDialog {
                background-color: #fafafa;
            }
            QGroupBox {
                border: 1px solid #e0e0e0;
                border-radius: 4px;
                margin-top: 10px;
                font-family: "Roboto", sans-serif;
                font-size: 14px;
                font-weight: 500;
                color: #333;
            }
            QGroupBox::title {
                subcontrol-origin: margin;
                subcontrol-position: top left;
                padding: 0 8px;
            }
        """
        dialog.setStyleSheet(style_sheet)

    def on_naming_rule_changed(self, index, item):
        """当命名规则改变时更新命名规则"""
        folder_path = item.text(1)
        combo_box = self.settings_dialog.folder_tree.itemWidget(item, 2)
        if combo_box.currentText() == '自定义':
            # 弹出自定义对话框
            rule, ok = QInputDialog.getText(None, '自定义命名规则', '请输入命名规则（可使用 {date} 和 {file_name}）：')
            if ok and rule:
                # 将自定义规则插入到“自定义”项之前
                combo_box.insertItem(combo_box.count() - 1, rule)
                combo_box.setCurrentText(rule)  # 设置为当前规则
                self.custom_naming_rules.append(rule)  # 保存自定义规则
                self.naming_rules[folder_path] = rule  # 更新命名规则
            else:
                # 如果用户点击“取消”或未输入任何内容，恢复到之前的命名规则
                previous_rule = self.naming_rules.get(folder_path, '{date}-{file_name}')
                combo_box.setCurrentText(previous_rule)
        else:
            # 预设命名规则
            rule = combo_box.currentText()
            self.naming_rules[folder_path] = rule  # 更新命名规则

        # 动态更新监控范围
        if self.is_running:
            self.start_monitoring()

    def select_folder(self):
        folder_path = QFileDialog.getExistingDirectory(None, '选择监控文件夹', '')
        if folder_path:
            # 检查文件夹是否已经存在
            if folder_path in self.target_paths:
                self.logger.log_message(f"文件夹路径已存在: {folder_path}")
                return

            item = QTreeWidgetItem(self.settings_dialog.folder_tree)
            item.setText(1, folder_path)  # 文件夹名
            checkbox = QCheckBox()
            checkbox.setChecked(True)  # 默认选中
            checkbox.stateChanged.connect(lambda state, item=item: self.on_checkbox_state_changed(state, item))
            self.settings_dialog.folder_tree.setItemWidget(item, 0, checkbox)
            
            # 添加命名规则下拉菜单
            naming_rule_combo = QComboBox()
            naming_rule_combo.addItems(['{date}-{file_name}', '{file_name}-{date}', '{date}_{file_name}', '顺序命名'])
            naming_rule_combo.addItems(self.custom_naming_rules)  # 添加自定义命名规则
            naming_rule_combo.addItem('自定义')  # 始终将“自定义”放在最后
            naming_rule_combo.setCurrentText('{date}-{file_name}')
            naming_rule_combo.currentIndexChanged.connect(lambda index, item=item: self.on_naming_rule_changed(index, item))
            self.settings_dialog.folder_tree.setItemWidget(item, 2, naming_rule_combo)
            
            # 添加删除按钮
            delete_button = QPushButton("❎")
            delete_button.setStyleSheet("""
                QPushButton {
                    background-color: transparent;
                    border: none;
                    font-size: 14px;
                }
                QPushButton:hover {
                    color: #ff0000;
                }
                QPushButton:pressed {
                    color: #cc0000;
                }
            """)
            delete_button.clicked.connect(lambda: self.delete_folder_item(item))
            self.settings_dialog.folder_tree.setItemWidget(item, 3, delete_button)
            
            self.target_paths.append(folder_path)
            self.naming_rules[folder_path] = '{date}-{file_name}'  # 默认命名规则
            self.folder_states[folder_path] = True  # 默认选中
            self.logger.log_message(f"监控文件夹已添加: {folder_path}")
            self.logger.log_message(f"当前 target_paths: {self.target_paths}")  # 调试日志
        else:
            self.logger.log_message(f"文件夹路径无效: {folder_path}")  # 调试日志
            
            # 动态更新监控范围
            if self.is_running:
                self.start_monitoring()

    def delete_folder_item(self, item):
        """删除文件夹项"""
        folder_path = item.text(1)
        if folder_path in self.target_paths:
            self.target_paths.remove(folder_path)
            self.naming_rules.pop(folder_path, None)  # 删除文件夹的命名规则
            self.folder_states.pop(folder_path, None)  # 删除文件夹状态
            self.settings_dialog.folder_tree.takeTopLevelItem(self.settings_dialog.folder_tree.indexOfTopLevelItem(item))
            self.logger.log_message(f"监控文件夹已删除: {folder_path}")
            self.logger.log_message(f"当前 target_paths: {self.target_paths}")  # 调试日志
                    # 动态更新监控范围
            if self.is_running:
                self.start_monitoring()

    def on_folder_item_clicked(self, item):
        folder_path = item.text(1)
        if os.path.isdir(folder_path) and item.childCount() == 0:  # 避免重复加载子文件夹
            for subdir in os.listdir(folder_path):
                subdir_path = os.path.join(folder_path, subdir)
                if os.path.isdir(subdir_path):
                    child_item = QTreeWidgetItem(item)
                    child_item.setText(1, subdir_path)  # 文件夹名
                    checkbox = QCheckBox()
                    checkbox.setChecked(True)  # 默认选中
                    checkbox.stateChanged.connect(lambda state, item=child_item: self.on_checkbox_state_changed(state, item))
                    self.settings_dialog.folder_tree.setItemWidget(child_item, 0, checkbox)
                    
                    # 添加命名规则下拉菜单
                    naming_rule_combo = QComboBox()
                    naming_rule_combo.addItems(['{date}-{file_name}', '{file_name}-{date}', '{date}_{file_name}', '顺序命名'])
                    naming_rule_combo.addItems(self.custom_naming_rules)  # 添加自定义命名规则
                    naming_rule_combo.addItem('自定义')  # 始终将“自定义”放在最后
                    naming_rule_combo.setCurrentText('{date}-{file_name}')
                    naming_rule_combo.currentIndexChanged.connect(lambda index, item=child_item: self.on_naming_rule_changed(index, item))
                    self.settings_dialog.folder_tree.setItemWidget(child_item, 2, naming_rule_combo)
                    
                    # 添加删除按钮
                    delete_button = QPushButton("❎")
                    delete_button.setStyleSheet("""
                        QPushButton {
                            background-color: transparent;
                            border: none;
                            font-size: 14px;
                        }
                        QPushButton:hover {
                            color: #ff0000;
                        }
                        QPushButton:pressed {
                            color: #cc0000;
                        }
                    """)
                    delete_button.clicked.connect(lambda: self.delete_folder_item(child_item))
                    self.settings_dialog.folder_tree.setItemWidget(child_item, 3, delete_button)

                    # 递归加载子文件夹
                    self.load_subfolder_states(child_item, subdir_path)

    def on_checkbox_state_changed(self, state, item):
        """当复选框状态改变时，递归处理子文件夹"""
        folder_path = item.text(1)
        self.folder_states[folder_path] = state == Qt.Checked  # 更新文件夹状态
        checkbox = self.settings_dialog.folder_tree.itemWidget(item, 0)
        if checkbox.isChecked():
            # 如果选中父文件夹，自动选中所有子文件夹
            self.set_children_checked(item, True)
        else:
            # 如果取消选中父文件夹，自动取消选中所有子文件夹
            self.set_children_checked(item, False)
        
        # 动态更新监控范围
        if self.is_running:
            self.start_monitoring()

    def set_children_checked(self, parent_item, checked):
        """递归设置子文件夹的选中状态"""
        for i in range(parent_item.childCount()):
            child_item = parent_item.child(i)
            child_checkbox = self.settings_dialog.folder_tree.itemWidget(child_item, 0)
            child_checkbox.setChecked(checked)
            self.folder_states[child_item.text(1)] = checked  # 更新子文件夹状态
            # 递归处理子文件夹
            self.set_children_checked(child_item, checked)      

    def save_current_settings_as_template(self):
        file_path, _ = QFileDialog.getSaveFileName(None, "保存模板文件", "", "Template Files (*.template)")
        if file_path:
            template_data = (os.path.basename(file_path), self.target_paths, self.naming_rules, self.include_subfolders, self.folder_states, self.custom_naming_rules)
            with open(file_path, 'wb') as f:
                pickle.dump(template_data, f)
            self.templates.append(template_data)
            self.logger.log_message(f"模板已保存: {file_path}")
            QMessageBox.information(None, "提示", "模板保存成功")

    def show_template_menu(self):
        template_menu = QMenu('模板操作', self.menu)
        load_template_action = template_menu.addAction('读取模板')
        save_template_action = template_menu.addAction('保存模板')
        new_template_action = template_menu.addAction('新建模板')
        template_menu.addSeparator()
        close_template_menu_action = template_menu.addAction('关闭')

        load_template_action.triggered.connect(self.load_template)
        save_template_action.triggered.connect(self.save_template)
        new_template_action.triggered.connect(self.new_template)
        close_template_menu_action.triggered.connect(template_menu.close)

        self.menu.addMenu(template_menu)

    def load_template(self):
        file_path, _ = QFileDialog.getOpenFileName(None, "选择模板文件", "", "Template Files (*.template)")
        if file_path:
            try:
                with open(file_path, 'rb') as f:
                    template_data = pickle.load(f)
                    self.templates.append(template_data)
                    self.logger.log_message(f"模板已加载: {file_path}")
                    QMessageBox.information(None, "提示", "模板读取成功")
            except FileNotFoundError:
                self.logger.log_message("指定的模板文件不存在，请检查文件路径是否正确")
                QMessageBox.warning(None, "提示", "指定的模板文件不存在，请检查文件路径是否正确")
            except pickle.UnpicklingError:
                self.logger.log_message("模板文件格式有误，无法正确解析，请检查文件内容")
                QMessageBox.warning(None, "提示", "模板文件格式有误，无法正确解析，请检查文件内容")

    def save_template(self):
        if not self.template_path:
            file_path, _ = QFileDialog.getSaveFileName(None, "保存模板文件", "", "Template Files (*.template)")
        if file_path:
            self.template_path = file_path
            template_data = (os.path.basename(self.template_path), self.target_paths, self.naming_rules, self.include_subfolders, self.folder_states, self.custom_naming_rules)
            with open(self.template_path, 'wb') as f:
                pickle.dump(template_data, f)
            self.templates.append(template_data)
            self.logger.log_message(f"模板已保存: {file_path}")
            QMessageBox.information(None, "提示", "模板保存成功")

    def new_template(self):
        new_rule, ok = QInputDialog.getText(None, '新建模板', '请输入新的命名规则')
        if ok and new_rule:
            new_path, ok = QInputDialog.getText(None, '新建模板', '请输入要监控的文件夹路径')
            if ok and new_path:
                if not os.path.exists(new_path):
                    self.logger.log_message("输入的文件夹路径不存在，请重新输入")
                    QMessageBox.warning(None, "提示", "输入的文件夹路径不存在，请重新输入")
                    return
                file_path, _ = QFileDialog.getSaveFileName(None, "保存新建模板文件", "", "Template Files (*.template)")
                if file_path:
                    template_data = (os.path.basename(file_path), [new_path], {new_path: new_rule}, {new_path: True}, {new_path: True}, [new_rule])
                    with open(file_path, 'wb') as f:
                        pickle.dump(template_data, f)
                    self.templates.append(template_data)
                    self.logger.log_message(f"新建模板已保存: {file_path}")
                    QMessageBox.information(None, "提示", "新建模板保存成功")

    def load_templates(self):
        try:
            with open('templates.template', 'rb') as f:
                self.templates = pickle.load(f)
        except FileNotFoundError:
            self.templates = []

    def exit_app(self):
        self.stop_auto_naming()
        if self.observer:
            self.observer.stop()
            self.observer.join()
        with open('templates.template', 'wb') as f:
            pickle.dump(self.templates, f)
        self.logger.save_log_history("程序退出")  # 退出时保存日志
        self.save_last_settings()  # 退出时保存当前设置
        self.app.quit()

    def show_start_monitor_notification(self):
        """显示监控启动通知"""
        self.tray.showMessage(
            "监控已启动",
            "文件夹监控已启动，自动命名功能已启用。",
            QSystemTrayIcon.Information,  # 通知类型
            1000  # 通知显示时间（毫秒）
        )


class CustomDialog(QDialog):
    def __init__(self, parent_widget=None):
        super().__init__()
        self.parent_widget = parent_widget  # 保存父窗口的引用
        self.last_modified_time = 0  # 记录文件最后修改时间
        self.folder_tree = None  # 初始化 folder_tree 属性

    def load_log_history(self):
        """从文件加载日志历史并显示到表格中"""
        if os.path.exists('log_history.txt'):
            current_modified_time = os.path.getmtime('log_history.txt')
            if current_modified_time != self.last_modified_time:  # 仅在文件发生变化时读取
                self.last_modified_time = current_modified_time
                with open('log_history.txt', 'r', encoding='utf-8') as f:
                    log_content = f.read().splitlines()
                    self.parent_widget.log_table.setRowCount(len(log_content))
                    for row, log_entry in enumerate(log_content):
                        parts = log_entry.split(' - ')
                        if len(parts) >= 2:
                            timestamp, message = parts[0], parts[1]
                            self.parent_widget.log_table.setItem(row, 0, QTableWidgetItem(timestamp))
                            self.parent_widget.log_table.setItem(row, 1, QTableWidgetItem(message))

    def update_log_display(self, log_entry):
        """更新日志显示"""
        parts = log_entry.split(' - ')
        if len(parts) >= 2:
            row = self.parent_widget.log_table.rowCount()
            self.parent_widget.log_table.insertRow(row)
            self.parent_widget.log_table.setItem(row, 0, QTableWidgetItem(parts[0]))
            self.parent_widget.log_table.setItem(row, 1, QTableWidgetItem(parts[1]))

    def closeEvent(self, event):
        self.hide()
        event.ignore()


if __name__ == "__main__":
    if run_as_admin():  # 提升权限
        app = TrayApp()
        sys.exit(app.app.exec_())